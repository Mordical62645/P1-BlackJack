...
class Game:
    ...
    def __init__(self):
        ...
        self.resolution = pygame.display.set_mode((800, 480))
        self.resolution.fill('Green')
        self.fps = pygame.time.Clock()
        self.title_font = pygame.font.Font("font/KarenFat.ttf", 50)
        self.ground = pygame.image.load("graphics/ground.jpg")
        self.title_surface = self.title_font.render("BlackJack", False, "Black")
        self.card_deck = pygame.image.load("graphics/Card_Deck/card_deck.png")
        self.deck = self.deck_func()
    ...
    def deck_func():
        ...
        deck = [self.Card(suit, value) for suit in suits for value in values]
        random.shuffle(deck)
        return deck
    ...
    def deal_initial_cards(deck):
        ...
        for _ in range(2):
            ...
            self.playerscore.append(self.get_card_value(player_card))
            self.player_hand.append(player_card)

            dealer_card = self.deck.pop()
            self.dealerscore.append(self.get_card_value(dealer_card))
            self.dealer_hand.append(dealer_card)

    def get_card_value(self, card):
        if str(card.value) in ["J", "Q", "K"]:
            return 10
        elif str(card.value) == "A":
            return 11
        else:
            return card.value
